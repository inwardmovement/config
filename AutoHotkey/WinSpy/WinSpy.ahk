...
hSysMenu := DllCall("GetSystemMenu", "Ptr", hSpyWnd, "Int", False)
...
LoadCursors() {
    ...
    For Key, Value in Constants {
        hCursor := DllCall("LoadCursor", "Ptr", 0, "UInt", Value)
        Cursors[hCursor] := Key
    }
}
...
LoadExtraInfo:
    ...
    Else If (InStr(Class, "Scintilla")) {
        ...
        LV_Add("", "Text length", SendMsg(2006) " bytes") ; SCI_GETLENGTH
        LV_Add("", "Modified", SendMsg(2159)) ; SCI_GETMODIFY
        LV_Add("", "Current position", Pos := SendMsg(2008) + 1) ; SCI_GETCURRENTPOS
        LV_Add("", "Current line", SendMsg(2166, Pos) + 1) ; SCI_LINEFROMPOSITION
        LV_Add("", "Current column", SendMsg(2129, Pos)) ; SCI_GETCOLUMN
        LV_Add("", "Line count", SendMsg(2154)) ; SCI_GETLINECOUNT
        LV_Add("", "Read only", SendMsg(2140)) ; SCI_GETREADONLY
        ...
        LV_Add("", "Indent with spaces", !SendMsg(2125)) ; SCI_GETUSETABS
        LV_Add("", "Show indentation guides", SendMsg(2133)) ; SCI_GETINDENTATIONGUIDES
        LV_Add("", "Anchor position", SendMsg(2009)) ; SCI_GETANCHOR
        LV_Add("", "Selection mode", SendMsg(2423)) ; SCI_GETSELECTIONMODE
        LV_Add("", "Selection start", SendMsg(2143)) ; SCI_GETSELECTIONSTART
        LV_Add("", "Selection end", SendMsg(2145)) ; SCI_GETSELECTIONEND
        LV_Add("", "Multiple selection", SendMsg(2564)) ; SCI_GETMULTIPLESELECTION
        ...
        LV_Add("", "Word start position", SendMsg(2266, Pos, 1)) ; SCI_WORDSTARTPOSITION
        LV_Add("", "Word end position", SendMsg(2267, Pos, 1)) ; SCI_WORDENDPOSITION
...
SetButtonIcon(hButton, File, Index := 1) {
    himl := DllCall("ImageList_Create", "Int", 16, "Int", 16, "UInt", 0x20, "Int", 1, "Int", 1) ; ILC_COLOR32
...
ShowSettingsDlg:
    ...
    Gui Add, Button, gApplySettings x81 y243 w84 h24 +Default, &OK
    Gui Add, Button, gSettingsClose x171 y243 w84 h24, &Cancel
...
ChooseBorderColor:
    If (ChooseColor(g_BorderColor, hSettingsDlg)) {
        GuiControl, Settings: +c%g_BorderColor%, BorderColorPreview
    }
Return
...
ShowFindDlg:
    If (FindDlgExist) {
        ...
        Gui Add, Button, gFindOK x194 y342 w88 h25 Default, &OK
        Gui Add, Button, gFindClose x293 y342 w88 h25, &Cancel
...
LoadProcessProperties:
    ...
    If (Enum[Process]) {
        ...
        If (!hIcon || ExePath == "") {
            hIcon := DllCall("LoadIcon", "Ptr", 0, "UInt", 32512) ; IDI_APPLICATION
        }
